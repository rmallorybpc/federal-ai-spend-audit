...
- `.github/workflows/deploy-pages.yml` deploys `site/` to Pages via GitHub
  ...
  run fails.
- `site/series-data.js` is generated by `analyze.py` and read by
  `site/index.html` to draw the adoption-curve chart. Do not hand-edit it;
  regenerate it by running `analyze.py`. The page must degrade gracefully if it
  is missing (show a "run analyze" note), and must never hardcode the curve.
